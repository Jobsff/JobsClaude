...
* **纠错即投资**：每当遇到错误、逻辑翻车或用户纠正你的行为时，必须立即执行：
  1. 分析错误根源
  2. 将该教训写入 `tasks/lessons.md`
  3. 告知用户："我已将此教训存入 lessons.md，未来将永久避免此类错误"

* **目标**：确保单次纠错优化所有未来任务的默认行为。

* **自主 Bug 修复**：
  - 收到 bug 报告时直接修复，无需手把手指导
  - 指向日志、错误、失败的测试并解决它们
  - 用户无需上下文切换
  - 主动修复失败的 CI 测试
...
* **验证方法**：
  ...
  - 必要时对比修改前后的行为差异
  - 自问："高级工程师会批准这个吗？"
* **不验证不标记完成**：永远不要在没有证明的情况下标记任务完成

---

## 9. 开发日志 (HISTORY_LOG.md)
...
| 日期 | 版本 | 更新内容 |
|------|------|----------|
| 2026-03-08 | v1.4.0 | lessons.md 路径变更、新增"自主 Bug 修复"和"验证标准"章节 |
